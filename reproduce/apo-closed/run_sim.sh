...
do
	...
	if [ "$i" -eq 1 ]; then
		...
		cp ${prev}/md_run/apo_closed.gro ${current}/equilibrate/cpd_initial.gro

        fi
	
	cd ${current}/equilibrate

	if [ -f ${current}/md_run/apo_closed.cpt ]; then

		cd ${current}/md_run
		gmx_mpi mdrun -cpi apo_closed -deffnm apo_closed -nb gpu -update gpu -pme gpu -pin off -ntomp $SLURM_CPUS_PER_TASK -nobackup
		...
		gmx grompp -f ${MDP}/Production.mdp -c npt.gro -r npt.gro -t npt.cpt -p topol.top -o apo_closed.tpr -nobackup
		gmx_mpi mdrun -deffnm apo_closed -nb gpu -update gpu -pme gpu -pin off --ntomp $SLURM_CPUS_PER_TASK -nobackup

	fi

	if [ -f apo_closed.gro ]; then
