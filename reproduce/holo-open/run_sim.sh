...
do
	...
	if [ "$i" -eq 1 ]; then
		...
		cp ${prev}/md_run/holo_open.gro ${current}/equilibrate/cpd_initial.gro

		cd ${current}/equilibrate

	fi

	if [ -f ${current}/md_run/holo_open.cpt ]; then

		cd ${current}/md_run
		gmx_mpi mdrun -cpi holo_open -deffnm holo_open -nb gpu -update gpu -pme gpu -pin off -ntomp $SLURM_CPUS_PER_TASK -nobackup
		...
		gmx grompp -f ${MDP}/Production.mdp -n index.ndx -c npt.gro -r npt.gro -t npt.cpt -p topol.top -o holo_open.tpr -nobackup
		gmx_mpi mdrun -deffnm holo_open -nb gpu -update gpu -pme gpu -pin off -ntomp $SLURM_CPUS_PER_TASK -nobackup

	fi

	if [ -f holo_open.gro ]; then
