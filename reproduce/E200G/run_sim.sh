...
do
        ...
        if [ "$i" -eq 1 ]; then
                ...
                cp ${prev}/md_run/E200G.gro ${current}/equilibrate/cpd_initial.gro

        fi

        cd ${current}/equilibrate

        if [ -f ${current}/md_run/E200G.cpt ]; then

                cd ${current}/md_run
                gmx_mpi mdrun -cpi E200G -deffnm E200G -nb gpu -update gpu -pme gpu -pin off -ntomp $SLURM_CPUS_PER_TASK -nobackup
                ...
                gmx grompp -f ${MDP}/Production.mdp -c npt.gro -r npt.gro -t npt.cpt -p topol.top -o E200G.tpr -nobackup
                gmx_mpi mdrun -deffnm E200G -nb gpu -update gpu -pme gpu -pin off --ntomp $SLURM_CPUS_PER_TASK -nobackup
